...
// Simple HTTP Basic Auth check. Returns true if auth succeeds or auth is disabled.
// Set PULSE_PASSWORD in .env or environment to enable.
// Usage: if (!checkAuth(req)) return new Response("Unauthorized", { status: 401, headers: { "WWW-Authenticate": 'Basic realm="PAI Pulse"' } })
function checkAuth(req: Request): boolean {
  ...
  // Accept either "user:password" or just ":password" (empty username)
  const parts = userPass.split(":")
  const providedPassword = parts.length >= 2 ? parts[1] : parts[0]

  // Use constant-time comparison to avoid timing attacks (good enough for this use case)
...
async function main() {
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      // Auth check — applies to all routes except health endpoint, logo, and voice routes
      // Health endpoint, logo, and voice TTS notifications are always accessible
      const publicPaths = ["/healthz", "/pai-logo.png", "/favicon.ico", "/notify", "/notify/personality", "/voice"]
      if (!publicPaths.includes(pathname) && !checkAuth(req)) {
        return new Response("Unauthorized", {
          status: 401,
          headers: { "WWW-Authenticate": 'Basic realm="PAI Pulse Dashboard"' }
        })
      }
      ...
      // Static assets: logo, favicon (bypass Observability)
      if (pathname === "/pai-logo.png" || pathname === "/favicon.ico") {
        const publicPath = join(PULSE_DIR, "Observability", "public", pathname === "/pai-logo.png" ? "pai-logo.png" : "favicon.ico")
        try {
          const file = readFileSync(publicPath)
          return new Response(file, {
            headers: {
              "Content-Type": pathname === "/pai-logo.png" ? "image/png" : "image/x-icon",
