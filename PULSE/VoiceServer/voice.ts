...
async function playAudio(audioBuffer: ArrayBuffer, volume: number = FALLBACK_VOLUME): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isMac) {
      ...
      // Try paplay (PulseAudio) first, fall back to aplay (ALSA), then ffplay (ffmpeg)
      if (existsSync("/usr/bin/paplay")) {
        player = "/usr/bin/paplay"
        args = ["--volume", (volume * 100).toString(), tempFile]
      } else if (existsSync("/usr/bin/aplay")) {
        player = "/usr/bin/aplay"
        args = ["-v", volume.toString(), tempFile]
      } else if (existsSync("/usr/bin/ffplay")) {
        player = "/usr/bin/ffplay"
        args = ["-nodisp", "-nostats", "-autoexit", "-volume", volume.toString(), tempFile]
      } else {
        reject(new Error("No audio player found on this system"))
        return
      }
    } else {
